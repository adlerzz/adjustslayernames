...
export function uNameToBytes(text: string): [number, Buffer]{
    const bLength = Buffer.alloc(4);
    bLength.writeInt32BE(text.length);
    const bBody = Buffer.alloc(text.length*2);

    bBody.write(text, "utf16le");
    bBody.swap16();
        
    const bEnding = Buffer.alloc(2, 0);
    const bString = Buffer.concat([bLength, bBody, bEnding]);
    
    const size = bString.byteLength;
